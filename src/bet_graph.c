...
struct _struct_bet_graph_data
{
    ...
    gdouble tab_X[MAX_POINTS_HISTOGRAM];
    gdouble tab_Y[MAX_POINTS_HISTOGRAM];

    /* données pour les camemberts */
    gint type_infos;    /* 0 type crédit ou < 0, 1 type débit ou >= 0, -1 tous types */
    gchar tab_libelle[MAX_POINTS_HISTOGRAM][TAILLE_MAX_LIBELLE];
...
static void struct_free_bet_line_data ( struct_bet_graph_data *self );
static  void struct_free_bet_sectors_data ( struct_bet_sectors_data *self );
static struct_bet_graph_data *struct_initialise_bet_line_data ( void );
static struct_bet_sectors_data *struct_initialise_bet_sectors_data ( void );
/*END_STATIC*/
...
void bet_graph_line_graph_new ( GtkWidget *menu_item, GtkTreeView *tree_view )
{
    ...
    self = struct_initialise_bet_line_data ( );
    ...
    /* free the data */
    struct_free_bet_line_data ( self );
    gtk_widget_destroy ( dialog );
}
...
/*                 }                  */
            ...
            if ( self -> nbre_elemnts >= MAX_POINTS_HISTOGRAM )
                break;
        }
        while ( gtk_tree_model_iter_next ( GTK_TREE_MODEL ( model ), &iter ) );
 ...
 *
 * */
struct_bet_graph_data *struct_initialise_bet_line_data ( void )
{
    struct_bet_graph_data *self;
    gint i;
    ...
    self -> tab_vue_libelle = g_malloc ( MAX_POINTS_HISTOGRAM * sizeof ( gchar* ) );

    for ( i = 0; i < MAX_POINTS_HISTOGRAM; i++ )
    {
        self -> tab_vue_libelle[i] = self -> tab_libelle[i];
    }
...
void struct_free_bet_line_data ( struct_bet_graph_data *self )
{
    g_free ( self -> title );
    g_free ( self -> tab_vue_libelle );
