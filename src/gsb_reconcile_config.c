...
/*START_STATIC*/
static gboolean gsb_reconcile_config_move_down ( GtkWidget *button,
					  GtkWidget *tree_view );
static gboolean gsb_reconcile_config_move_up ( GtkWidget *button,
					GtkWidget *tree_view );
static gboolean gsb_reconcile_config_neutral_toggled ( GtkCellRendererToggle *cell, 
						gchar *path_str,
						GtkWidget *tree_view );
static gboolean gsb_reconcile_config_payment_toggled ( GtkCellRendererToggle *cell, 
						gchar *path_str,
						GtkWidget *tree_view );
static gboolean gsb_reconcile_config_select ( GtkTreeSelection *tselection, 
				       GtkTreeModel *model );
...
GtkWidget *gsb_reconcile_config_create ( void )
{
    ...
    g_signal_connect (reconcile_selection, "changed", 
		      G_CALLBACK (gsb_reconcile_config_select), reconcile_model);
    ...
    gtk_tree_view_column_set_attributes (column, cell,
					 ...
					 "sensitive", RECONCILIATION_SENSITIVE_COLUMN,
					 NULL);
    ...
    g_signal_connect (cell, "toggled", 
		      G_CALLBACK (gsb_reconcile_config_payment_toggled), reconcile_treeview);
    ...
    gtk_cell_renderer_toggle_set_radio ( GTK_CELL_RENDERER_TOGGLE(cell), FALSE );
    g_signal_connect (cell, "toggled", 
		      G_CALLBACK (gsb_reconcile_config_neutral_toggled), reconcile_treeview);
    g_object_set (cell, "xalign", 0.5, NULL);
    column = gtk_tree_view_column_new ( );
    gtk_tree_view_column_set_alignment ( column, 0.5 );
    gtk_tree_view_column_pack_end ( column, cell, TRUE );
    ...
    g_signal_connect ( GTK_OBJECT ( button_move_up ), "clicked",
		       G_CALLBACK (gsb_reconcile_config_move_up), reconcile_treeview );
    ...
    g_signal_connect ( GTK_OBJECT ( button_move_down ), "clicked",
		       G_CALLBACK (gsb_reconcile_config_move_down), reconcile_treeview);
    gtk_box_pack_start ( GTK_BOX (vbox_fleches_tri),
			 button_move_down,
			 FALSE, FALSE, 0 );
...
void gsb_reconcile_config_fill ( void )
{
	...
	gtk_tree_store_set (GTK_TREE_STORE (model), &account_iter,
			    ...
			    RECONCILIATION_TYPE_COLUMN, 0,
			    RECONCILIATION_SENSITIVE_COLUMN, visible,
			    -1 );
 ...
 * \return FALSE
 */
gboolean gsb_reconcile_config_select ( GtkTreeSelection *tselection, 
				       GtkTreeModel *model )
{
    GtkTreeIter iter;
    gboolean good;
    ...
    if (good)
    {
	gboolean payment_method;

	gtk_tree_model_get (model, &iter, 
			    RECONCILIATION_TYPE_COLUMN, &payment_method,
			    -1 );
 ...
 * \return FALSE
 */
gboolean gsb_reconcile_config_move_up ( GtkWidget *button,
					GtkWidget *tree_view )
{
    GtkTreePath * treepath;
    ...
    gsb_reconcile_config_select ( selection, 
				  GTK_TREE_MODEL(model) );
    gsb_reconcile_config_update_account_sort_list (account_number, model);
    return FALSE;
}
 ...
 * \return FALSE
 */
gboolean gsb_reconcile_config_move_down ( GtkWidget *button,
					  GtkWidget *tree_view )
{
    GtkTreePath * treepath;
    gboolean good, visible;
 ...
 * \return FALSE
 */
gboolean gsb_reconcile_config_payment_toggled ( GtkCellRendererToggle *cell, 
						gchar *path_str,
						GtkWidget *tree_view )
{
    GtkTreePath * treepath;
 ...
 * \return FALSE
 */
gboolean gsb_reconcile_config_neutral_toggled ( GtkCellRendererToggle *cell, 
						gchar *path_str,
						GtkWidget *tree_view )
{
    GtkTreePath * treepath;
