													  ...
													  gpointer null)
{
    ...
    tmpstr = g_strdup_printf (_("You are changing the currency of the account, do you want to change "
								"the currency of the transactions too?\n(yes will change all the "
								"transactions currency from %s to %s, all the transactions with another "
								"currency will stay the same).\n\nArchived and reconcilied transactions "
								"will be left unmodified."),
							  gsb_data_currency_get_name (account_currency_number),
							  gsb_data_currency_get_name (new_currency_number));
    result = dialogue_yes_no (tmpstr, _("Change the transactions currency"), GTK_RESPONSE_NO);
    g_free (tmpstr);
...
static gboolean gsb_account_property_iban_set_bank_from_iban (gchar *iban)
{
    ...
    {
        gsb_account_property_iban_display_bank_data (current_account, bank_number);
...
void gsb_account_property_set_label_code_banque (gint bank_number)
{
    ...
    {
		gtk_label_set_text (GTK_LABEL (label_code_banque), gsb_data_bank_get_code (bank_number));
...
void gsb_account_property_set_label_code_bic (gint bank_number)
{
    ...
    {
        gtk_label_set_text (GTK_LABEL (label_code_bic), gsb_data_bank_get_bic (bank_number));
