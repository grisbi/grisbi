...
struct _ArchiveStruct	/* Describe a archive */
 {
	gint archive_number;
	gchar *archive_name;

	/* for the archive created by date, else the 2 values are NULL */
	GDate *beginning_date;
	GDate *end_date;

	/* for the archive created by financial year, else the value is 0 */
	gint fyear_number;

	/* if created by report, this is the title of the report */
	gchar *report_title;
};
...
static gpointer gsb_data_archive_get_structure (gint archive_number)
{
	...
	if (archive_buffer && archive_buffer->archive_number == archive_number)
...
gboolean gsb_data_archive_remove (gint archive_number)
{
	...
	while (tmp_list)
	{
		...
		if (gsb_data_transaction_get_archive_number (transaction_number) == archive->archive_number)
			gsb_data_transaction_set_archive_number (transaction_number, 0);

		tmp_list = tmp_list->next;
	}

	/* remove the archive from the list */
	archive_list = g_slist_remove (archive_list, archive);
