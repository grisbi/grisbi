/* ************************************************************************** */
/* Fichier qui gère la liste des comptes et la partie gauche de l'onglet      */
/* opérations                                                                 */
...
GtkWidget *creation_liste_comptes (void)
{
  ...
  gtk_frame_set_shadow_type ( GTK_FRAME ( frame_label_compte_courant ), GTK_SHADOW_ETCHED_OUT );
  gtk_box_pack_start ( GTK_BOX ( onglet ), frame_label_compte_courant, FALSE, TRUE, 0 );
  ...
  gtk_misc_set_alignment ( GTK_MISC ( label_compte_courant ), MISC_HORIZ_CENTER, MISC_VERT_CENTER);
  gtk_container_add ( GTK_CONTAINER ( frame_label_compte_courant ), label_compte_courant );
  gtk_widget_show ( label_compte_courant );

  /* Création de la fenêtre des comptes qui peut contenir des barres
     de défilement si nécessaire */
  ...
  vbox_liste_comptes = gtk_vbox_new ( FALSE, 10 );
  gtk_scrolled_window_add_with_viewport ( GTK_SCROLLED_WINDOW ( scrolled_window ), vbox_liste_comptes );
  gtk_viewport_set_shadow_type ( GTK_VIEWPORT ( GTK_BIN ( scrolled_window ) -> child ), GTK_SHADOW_NONE );
	  ...
	  if ( COMPTE_CLOTURE )
	    {
	      ...
	      gtk_widget_set_sensitive ( GTK_WIDGET ( menu_comptes[3].widget ), TRUE );
	    }
	  else
  ...
  gtk_frame_set_shadow_type ( GTK_FRAME ( frame_equilibrage ), GTK_SHADOW_ETCHED_IN );
  gtk_box_pack_start ( GTK_BOX ( onglet ), frame_equilibrage, FALSE, TRUE, 0 );
  gtk_widget_show ( frame_equilibrage );

  vbox_frame_equilibrage = gtk_vbox_new ( FALSE, 5 );
  gtk_container_add ( GTK_CONTAINER  ( frame_equilibrage ), vbox_frame_equilibrage );
  ...
  gtk_box_pack_start ( GTK_BOX ( vbox_frame_equilibrage ), label_releve, FALSE, TRUE, 0 );
  ...
  gtk_box_pack_start ( GTK_BOX ( vbox_frame_equilibrage ), bouton, FALSE, TRUE, 0 );
...
/* ************************************************************************** */
/* Fonction qui renvoie un widget contenant un bouton de compte associé       */
/* à son nom                                                                  */
/* ************************************************************************** */
GtkWidget *comptes_appel ( gint no_de_compte )
{
  ...
  win_icones = gtk_hbox_new ( FALSE, 10);

  /* On crée le bouton contenant le livre fermé et ouvert,
     seul le fermé est affiché pour l'instant */
  ...
  gtk_button_set_relief ( GTK_BUTTON ( bouton ), GTK_RELIEF_NONE );
  gtk_box_pack_start ( GTK_BOX ( win_icones ), bouton, FALSE, TRUE, 0 );
  gtk_widget_show ( bouton );
  ...
  icone = gnome_stock_pixmap_widget ( GTK_WIDGET ( bouton ), GNOME_STOCK_PIXMAP_BOOK_BLUE );
  gtk_box_pack_start ( GTK_BOX ( hbox ), icone, FALSE, FALSE, 0 );
  ICONE_FERMEE = icone;
  gtk_widget_show ( icone );

  /* création de l'icone ouverte */

  icone = gnome_stock_pixmap_widget ( GTK_WIDGET ( bouton ), GNOME_STOCK_PIXMAP_BOOK_OPEN);
  gtk_box_pack_start ( GTK_BOX ( hbox ), icone, FALSE, FALSE, 0 );
  ICONE_OUVERTE = icone;
  ...
  gtk_box_pack_start ( GTK_BOX ( win_icones ), label, FALSE, TRUE, 0 );
  ...
  if ( ( menu = creation_menu_types ( 1, compte_courant, 0 )))
    {
      ...
      gtk_widget_set_sensitive ( widget_formulaire_operations[TRANSACTION_FORM_TYPE], TRUE );
      gtk_option_menu_set_menu ( GTK_OPTION_MENU ( widget_formulaire_operations[TRANSACTION_FORM_TYPE] ), menu );
      ...
      gtk_widget_set_sensitive ( widget_formulaire_operations[TRANSACTION_FORM_TYPE], FALSE );
...
void reaffiche_liste_comptes ( void )
{
  ...
  /* Création d'une icone et du nom par compte,
     et placement dans la liste selon l'ordre désiré */
  ...
  do
      ...
      if ( COMPTE_CLOTURE )
	{
	 ...
	 ( menu + 1 ) -> type = GNOME_APP_UI_ENDOFINFO;
	 gnome_app_insert_menus ( GNOME_APP ( window ), _("Accounts/Closed accounts/"), menu );
	 gtk_widget_set_sensitive ( GTK_WIDGET ( menu_comptes[3].widget ), TRUE );
	}
      else
	gtk_widget_show ( bouton );
