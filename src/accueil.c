...
static void update_liste_echeances_auto_accueil (gboolean force)
{
    ...
    if (scheduled_transactions_taken)
    {
		...
		while (pointeur_liste)
		{
			...
			{
				tmp_str2 = utils_real_get_string_with_currency (gsb_data_transaction_get_amount (transaction_number),
															    currency_number,
															    TRUE);
				tmp_str = g_strdup_printf (_("%s credited on %s"),
										   tmp_str2,
			...
			{
				tmp_str2 = utils_real_get_string_with_currency (gsb_real_abs (gsb_data_transaction_get_amount
																			  (transaction_number)),
															    currency_number,
															    TRUE);
				tmp_str = g_strdup_printf (_("%s debited on %s"),
    ...
    while (list_tmp)
    {
			...
			&& !gsb_data_account_get_mini_balance_authorized_message (i))
		{
			if (gsb_real_cmp (gsb_data_account_get_current_balance (i),
							  gsb_data_account_get_mini_balance_wanted (i)) == -1)
			{
				liste_autorise_et_voulu = g_slist_append (liste_autorise_et_voulu, gsb_data_account_get_name (i));
    ...
    if (liste_autorise_et_voulu)
    {
		...
		else
		{
			...
			while (liste_tmp)
			{
				...
				texte_affiche = g_strconcat (old_str, liste_tmp -> data, "\n", NULL);
				g_free (old_str);
