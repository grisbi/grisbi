...
gboolean gsb_data_scheduled_is_transfer (gint scheduled_number)
{
    ...
    if (scheduled->category_number || scheduled->split_of_scheduled)
		return FALSE;
...
gint gsb_data_scheduled_new_scheduled_with_number (gint scheduled_number)
{
    ...
    scheduled_list = g_slist_append (scheduled_list, scheduled);
...
gint gsb_data_scheduled_new_white_line (gint mother_scheduled_number)
{
    ...
    white_scheduled_list = g_slist_append (white_scheduled_list, scheduled);
										 ...
										 gboolean return_number)
{
    ...
    if (!scheduled || !scheduled->split_of_scheduled)
		return NULL;
    ...
    while (tmp_list)
    {
		...
		if (tmp_scheduled->mother_scheduled_number == scheduled_number)
		{
			if (return_number)
				...
				children_list = g_slist_append (children_list, tmp_scheduled);
		}
		tmp_list = tmp_list->next;
    }
															  ...
															  gint return_exponent)
{
    ...
    if (scheduled->currency_number == return_currency_number)
        return gsb_real_adjust_exponent  (scheduled->scheduled_amount, return_exponent);
											...
											gint target_scheduled_number)
{
    ...
    if (target_scheduled->method_of_payment_content)
        target_scheduled->method_of_payment_content = my_strdup (source_scheduled->method_of_payment_content);
    return TRUE;
}
