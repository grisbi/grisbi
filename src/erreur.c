/* ************************************************************************** */
/*  Fichier qui contient toutes les procédures relatives aux erreurs          */
...
/* ************************************************************************** */
/* Fonction appelée lors de la demande de fermeture du fichier de compte      */
/* ************************************************************************** */
...
{
  win_erreur = gnome_warning_dialog_parented ( texte_dialogue ,GTK_WINDOW ( window) );
...
gboolean blocage_boites_dialogues ( GtkWidget *dialog,
				    gpointer null )
{
  gtk_signal_emit_stop_by_name ( GTK_OBJECT ( dialog ), "delete_event" );
  return ( TRUE );
}
/* ************************************************************************** */
...
/* ************************************************************************** */
/* Fonction question :                                                        */
/* affiche le texte donné en argument et attend l'appui sur oui ( renvoie     */
...
gboolean question ( gchar *texte )
{
  ...
  dialogue = gnome_dialog_new ( _("Warning"),
  ...
  gtk_box_pack_start ( GTK_BOX ( GNOME_DIALOG ( dialogue ) -> vbox ), label, TRUE, TRUE, 0 );
		       ...
		       gchar *question )
{
  ...
  gnome_dialog_editable_enters ( GNOME_DIALOG ( dialog ), GTK_EDITABLE ( entree ));
  gtk_window_set_focus ( GTK_WINDOW ( dialog ), entree );
  gtk_box_pack_start ( GTK_BOX ( GNOME_DIALOG ( dialog ) -> vbox ), entree, FALSE, FALSE, 0 );
...
void affiche_log_message ( void )
{
  ...
  gtk_box_pack_start ( GTK_BOX ( GNOME_DIALOG ( dialog ) -> vbox ), scrolled_window, FALSE, FALSE, 0 );
...
void traitement_sigsegv ( gint signal_nb )
{
  ...
  if ( nom_fichier_comptes )
    ...
    nom_fichier_comptes = g_strconcat ( home_dir, _("/#grisbi_crash_no_name#"),NULL );
